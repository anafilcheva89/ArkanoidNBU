...
const int PADDLE_SPEED = 10.0f;
const int BALL_SPEED = 5.8f;
const int PADDLE_START_X = (SCREEN_WIDTH - 100) / 2;
const int PADDLE_START_Y = 550;

TTF_Font *font = NULL;
TTF_Font *font1 = NULL;
SDL_Color text_color = {20,0,150};
...
SDL_Surface *game_over_text = NULL;
SDL_Surface *you_win_text = NULL;
...
bool initialize(){
	...
	font = TTF_OpenFont("libertylegion.ttf", 18);
	font1 = TTF_OpenFont("libertylegion.ttf", 48);

	background = loadImage("Graphics/borders.bmp");
	red_brick = loadImage("Graphics/red.bmp");
	blue_brick = loadImage("Graphics/blue.bmp");
	green_brick = loadImage("Graphics/green.bmp");
	paddle_image = loadImage("Graphics/paddle.png");
	ball_image = loadImage("Graphics/ball.png");
...
void displayText()
{
	...
	applySurface(1,550,level_text, screen);

	// Display Score
	current_score_text = TTF_RenderText_Solid(font,("Score:" + convertInt(score)).c_str(),text_color);
	applySurface(1,450,current_score_text, screen);

		// Display Lives Left
	lives_left_text = TTF_RenderText_Solid(font,("Lives:" + convertInt(lives_left)).c_str(),text_color);
	applySurface(1,500,lives_left_text, screen);

	
	if (lives_left == 0 ){

		game_over_text = TTF_RenderText_Solid(font1,("Game Over :("),text_color);
		applySurface(250,320,game_over_text, screen);
	}

		if (level > 3 )
	{
		you_win_text = TTF_RenderText_Solid(font1,("You Win!!!"),text_color);
		applySurface(250,320,you_win_text, screen);
	}
	...
	applySurface(1,1,high_score_text, screen);
